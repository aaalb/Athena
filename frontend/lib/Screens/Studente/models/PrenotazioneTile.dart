...
import 'package:frontend/Screens/Studente/Prenotazioni2/prenotazioni_screen.dart';
...
class CancellaPrenotazioneTile extends StatefulWidget
{
  ...
  @override
  State<CancellaPrenotazioneTile> createState() => CancellaPrenotazioneTileState(data: data, idprova: idprova);
}
...
class CancellaPrenotazioneTileState extends State<CancellaPrenotazioneTile>
{
  ...
  Widget tileText = Text('Annulla Prenotazione', textAlign: TextAlign.center,);
  AssetImage? okImage;
  ...
  static GlobalKey<PrenotazioniComponentState> prenotazioniKey = GlobalKey<PrenotazioniComponentState>();
      ...
      (
        ...
        onTap: ()
        {
          if(confirmState == 0)
          {
            ...
            {
              confirmState = 1;
              tileText = Image(image: warningImage!, fit: BoxFit.cover, color: Color.fromARGB(255, 209, 67, 67),);
              warningImage!.evict();
            });
            Future.delayed(Duration(milliseconds: 1750)).then((value)
            {
              setState(() {
                tileText = Text("Clicca per confermare", textAlign: TextAlign.center,);
                confirmState = 2;
              });
            });
          }
          else if(confirmState == 2)
          {
            ...
            _deleteAppello(this.idprova, this.data).then((value)
            {
              setState(() {
                if(value)
                {
                  tileText = Image(image: okImage!, fit: BoxFit.cover, color: Color.fromARGB(255, 209, 67, 67),);
                  okImage!.evict();
